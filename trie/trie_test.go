...
func testTrie(t *testing.T, trie PathTrie[int32]) {
	const firstPutValue = -1
	cases := []struct {
		key   string
		value int32
	}{
	...
	// get missing keys
	for _, c := range cases {
		if value := trie.Get(c.key); value != 0 {
...
func testNilBehavior(t *testing.T, trie PathTrie[int32]) {
	cases := []struct {
		key   string
		value interface{}
	}{
...
func testTrieRoot(t *testing.T, trie PathTrie[string]) {
	const firstPutValue = "first put"
	const putValue = "value"

	if value := trie.Get(""); value != "" {
...
func testTrieWalkPathError(t *testing.T, trie PathTrie[int]) {
